	...
	factory: (require) => {
		...
		const I18N = {
			zh: {
				...
				actToggleDetails: "切换详情栏",
				actToggleDetailsDesc: "展开或收起右侧详情栏",
			...
			en: {
				...
				actToggleDetails: "Toggle Details",
				actToggleDetailsDesc: "Expand or collapse the right details bar",
...
`;
		...
		function builtinDefs(ctx) {
			return [
				...
				{ id: "toggle-details", labelKey: "actToggleDetails", descriptionKey: "actToggleDetailsDesc", run: () => {
					const s = ctx.get("sidebarRight");
					if (!s || typeof s.toggleExpanded !== "function") return;
					try { s.toggleExpanded(); } catch (err) { console.error("[keys-palette] toggle-details failed:", err); }
				} },
