window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Local fallback for older DSH shells that predate the shared keycap
		// primitive. The slash identifies the palette's fixed Mod+/ shortcut.
		function KeycapIcon({ size = 16, className }) {
			return React.createElement("svg", {
				width: size, height: size, className, viewBox: "0 0 16 16", fill: "none",
				"aria-hidden": "true", xmlns: "http://www.w3.org/2000/svg",
			},
			React.createElement("rect", { x: 1.25, y: 1.75, width: 13.5, height: 12.5, rx: 2.25, stroke: "currentColor", strokeWidth: 1.3 }),
			React.createElement("path", { d: "M9.75 4.5L6.25 11.5", stroke: "currentColor", strokeWidth: 1.3, strokeLinecap: "round" }));
		}
		...
		let Modal = null;
		let TitleIcon = KeycapIcon;
		let IconClose = null;
		try {
			const primitives = require("@deepseek-ai/dsh-client-ui-primitives");
			Modal = primitives.Modal;
			// Newer shells export the same keycap used by the Shortcuts settings
			// nav row; retain the local drawing when running against an older shell.
			TitleIcon = primitives.IconKeycapOutline16 || KeycapIcon;
...
`;
		...
		function apply(ctx) {
			...
			function Palette() {
				...
				// Title row shared by both chrome paths: the rounded keycap at the
				// same 16px seat as the Shortcuts settings-nav glyph, the modal
				// title spec, and the settings dialog's 14px close glyph.
