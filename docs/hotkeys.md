...
| `toggle-details` | `sidebarRight.toggleExpanded()` — expand/collapse the right-hand Details sidebar. The right sidebar owns its presentation state; the layout store is only a downstream sync target, so the `sidebarRight` service is the correct face. |
